...
async function publish(skipVersioning = false) {
  try {
    ...
    if (!process.env.NPM_KEY) {
      throw new Error('NPM_KEY not found in environment variables. Please add it to your .env file.');
    }
    ...
    execSync(`npm config set //registry.npmjs.org/:_authToken ${process.env.NPM_KEY}`, { stdio: 'inherit' });
