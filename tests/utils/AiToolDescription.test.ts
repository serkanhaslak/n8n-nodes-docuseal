...
describe('AiToolDescription', () => {
	describe('aiToolOperations', () => {
		...
		it('should have generateDocument in options', () => {
			const operation = aiToolOperations[0];
			expect(operation).toBeDefined();
			const generateOption = operation!.options?.find((opt: any) => opt.value === 'generateDocument');
			
			expect(generateOption).toBeDefined();
			expect((generateOption as any)?.name).toBe('Generate Document');
			expect((generateOption as any)?.description).toBe('Generate a document using AI based on a description');
	...
	describe('aiToolFields', () => {
		...
		it('should have additionalOptions collection', () => {
			const additionalOptionsField = aiToolFields.find(field => field.name === 'additionalOptions');
		...
		it('should have language option in additionalOptions', () => {
			const additionalOptionsField = aiToolFields.find(field => field.name === 'additionalOptions');
			const languageOption = additionalOptionsField?.options?.find((opt: any) => opt.name === 'language') as any;
		...
		it('should have style option in additionalOptions', () => {
			const additionalOptionsField = aiToolFields.find(field => field.name === 'additionalOptions');
			const styleOption = additionalOptionsField?.options?.find((opt: any) => opt.name === 'style') as any;
		...
		it('should have fields option in additionalOptions', () => {
			const additionalOptionsField = aiToolFields.find(field => field.name === 'additionalOptions');
			const fieldsOption = additionalOptionsField?.options?.find((opt: any) => opt.name === 'fields') as any;
