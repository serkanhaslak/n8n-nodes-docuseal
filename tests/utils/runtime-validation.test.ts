...
import {
	...
	ValidationFactory,
	ValidationUtils,
} from '../../nodes/Docuseal/runtime-validation';
...
describe('runtime-validation', () => {
	describe('ValidationError', () => {
		...
		it('should create error with field and value', () => {
			const error = new ValidationError('Test error', 'email', 'invalid@');
			expect(error.message).toBe('Test error');
	...
	describe('UrlValidator', () => {
		...
		it('should require HTTPS when configured', () => {
			const validator = new UrlValidator({ requireHttps: true });
		...
		it('should reject private IPs when configured', () => {
			const validator = new UrlValidator({ allowPrivateIps: false });
		...
		it('should reject localhost when configured', () => {
			const validator = new UrlValidator({ allowLocalhost: false });
	...
	describe('ApiCredentialsValidator', () => {
		let validator: ApiCredentialsValidator;

		beforeEach(() => {
			validator = new ApiCredentialsValidator();
		});
		...
		it('should reject missing required fields', () => {
			const credentials = {
				productionApiKey: 'prod-key',
				// missing testApiKey and baseUrl
			};
	...
	describe('SubmitterValidator', () => {
		let validator: SubmitterValidator;

		beforeEach(() => {
			validator = new SubmitterValidator();
		});
	...
	describe('ValidationFactory', () => {
		...
		it('should create URL validator with options', () => {
			const validator = ValidationFactory.createUrlValidator({ requireHttps: true });
	...
	describe('ValidationUtils', () => {
		...
		it('should assert valid values without throwing', () => {
			const emailValidator = new EmailValidator();
			expect(() => {
				ValidationUtils.assert('test@example.com', emailValidator, 'email');
			}).not.toThrow();
		});
		...
		it('should throw ValidationError for invalid values', () => {
			const emailValidator = new EmailValidator();
			expect(() => {
				ValidationUtils.assert('invalid-email', emailValidator, 'email');
			}).toThrow(ValidationError);
		});

		it('should check validity without throwing', () => {
			const emailValidator = new EmailValidator();
			expect(ValidationUtils.isValid('test@example.com', emailValidator)).toBe(true);
			expect(ValidationUtils.isValid('invalid-email', emailValidator)).toBe(false);
		});
	});
});
